...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        ############################################################################
        all_dimes = hidden_dims.copy()
        all_dimes.insert(0, input_dim)
        all_dimes.append(num_classes)
        for i in range(1, len(all_dimes)):
            W1 = np.random.normal(scale=weight_scale, size=(all_dimes[i - 1], all_dimes[i]))
            b1 = np.zeros([all_dimes[i]])
            self.params['W' + str(i)] = W1
            self.params['b' + str(i)] = b1

            if self.use_batchnorm and i != len(all_dimes) - 1:
                self.params['beta' + str(i)] = np.zeros([all_dimes[i]])
                self.params['gamma' + str(i)] = np.ones([all_dimes[i]])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
