...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1 / N) * gamma * 1 / sqrtvar * (
        (N * dout) - np.sum(dout, axis=0) - (x_minus_mean) * np.square(ivar) * np.sum(dout * (x_minus_mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for ff in range(F):
            for hh in range(out_h):
                for ww in range(out_w):
                    out[nn, ff, hh, ww] = np.sum(w[ff, ...] * pad_x[nn, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW]) + b[ff]
...
def max_pool_forward_naive(x, pool_param):
    ...
    new_h = np.int((H - pool_height)/stride+1)
    new_w = np.int((W-pool_width)/stride+1)
        ...
        for cc in range(C):
            for hh in range(new_h):
                for ww in range(new_w):
                    out[nn, cc, hh, ww] = np.max(x[nn, cc, hh*stride:hh*stride+pool_height, ww*stride:ww*stride+pool_height])
