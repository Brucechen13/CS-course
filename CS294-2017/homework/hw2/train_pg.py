...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        out = tf.layers.dense(layer, output_size, activation=output_activation)
        return out
...
def train_PG(exp_name='',
    ...
    sy_adv_n = tf.placeholder(tf.float32, shape=[None], name="adv")
    ...
    if discrete:
        ...
        sy_sampled_ac = tf.random_normal(shape=tf.shape(sy_mean), mean=sy_mean, stddev=tf.exp(sy_logstd), seed=seed)
    ...
    if nn_baseline:
        ...
        # YOUR_CODE_HERE
        baseline_target = tf.placeholder(tf.float32, shape=[None], name="baseline")
    ...
    for itr in range(n_iter):
        ...
        if nn_baseline:
            ...
            sess.run([baseline_update_op], feed_dict={sy_ob_no:ob_no, baseline_target:q_n})
        ...
        # loss, _ = sess.run([loss, update_op], feed_dict=feed_dict)
